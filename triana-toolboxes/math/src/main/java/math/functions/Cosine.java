...
/**
 * A Cosine unit to apply the cosine function to the elements of an input data array. The array can be real or complex.
 * The returned data type will be real or complex as appropriate.
 * <p/>
 ...
 */
public class Cosine extends OldUnit {
    ...
    public String getPopUpDescription() {
        return "Applies the cosine function to the elements of the input data.";
    }

    /**
     * ********************************************* ** USER CODE of Cosine goes here    ***
     * *********************************************
     */
    public void process() {
        ...
        if (input instanceof GraphType) {
                    ...
                    if (complex) {
                        if (complexInput) {
                            ...
                            for (j = 0; j < inputdataI.length; j++) {
                                ...
                                inputdataR[j] = (d + drecip) * Math.cos(f) * 0.5;
                                inputdataI[j] = (drecip - d) * Math.sin(f) * 0.5;
                            }
                            ...
                            for (j = 0; j < inputdataI.length; j++) {
                                f = inputdataR[j] + phaseReal;
                                inputdataR[j] = offsetCosh * Math.cos(f);
                                inputdataI[j] = -offsetSinh * Math.sin(f);
                            }
                            ((GraphType) output).setDataArrayReal(tempR.restoreArray(false), dv);
                        ...
                        for (j = 0; j < inputdataR.length; j++) {
                            inputdataR[j] = Math.cos(inputdataR[j] + phaseReal);
                        }
                        ((GraphType) output).setDataArrayReal(tempR.restoreArray(false), dv);
                    }
                }
            ...
            if (complex) {
                if (complexInput) {
                    ...
                    r = (d + drecip) * Math.cos(f) * 0.5;
                    i = (drecip - d) * Math.sin(f) * 0.5;
                } else {
                    d = Math.exp(phaseImag);
                    drecip = 1.0 / d;
                    f = ((Const) input).getReal() + phaseReal;
                ...
                ((Const) output).setReal(Math.cos(((Const) input).getReal() + phaseReal));
            }
        }
    ...
    /**
     * @return a string containing the names of the types allowed to be input to Cosine, each separated by a white
     *         space.
     */
    ...
    /**
     * @return a string containing the names of the types output from Cosine, each separated by a white space.
     */
