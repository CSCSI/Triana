 ...
 * @see VariableDynamics
 */

public class VariableDynamicsEffect {
    ...
    public VariableDynamicsEffect(float threshold, double ratio, double attack, double release,
 ...
 *************************************/
    ...
    public short getRegionAverage(short[] currentChunk, int currentPosition) {
        ...
        for (int i = startPos; i < endPos; i++) {
            ...
            //	   System.out.println("currentChunk[i] = " + currentChunk[i]);
        }
        average = (short) (temp / threshWidth);
//		System.out.println("threshWidth = " + threshWidth);
        return average;
    }
    ...
    public short[] process(short input[]) {
        ...
        attackIncrement = (double) compressionRatio / (double) attackTime;
        releaseIncrement = (double) compressionRatio / (double) releaseTime;


//        attackIncrement = Math.pow(0.01, -1.0 / attackTime);
//		releaseIncrement = Math.pow(0.01, -1.0 / releaseTime );
        ...
        for (int n = 0; n < input.length; n++) {
            ...
            // Stops the compression ratio from going out of range
            if (currentCompressionRatio > compressionRatio) {
                currentCompressionRatio = compressionRatio;
            }

            // Stops the compression ratio from going out of range
            if (currentCompressionRatio < 1.0) {
            ...
            if (detection.equals("Peak")) {
                ...
                {
                    currentCompressionRatio = currentCompressionRatio + attackIncrement;
//                    System.out.println("currentCompressionRatio = " + currentCompressionRatio);
                    outputSample = (int) (input[n] / currentCompressionRatio);
                } else {
                    currentCompressionRatio = currentCompressionRatio - releaseIncrement;
                    outputSample = (int) (input[n] / currentCompressionRatio);
                }
            } else if (detection.equals("RMS")) {

                if (Math.abs(getRegionRMS(input, n)) > (int) thresholdLevel)  //switch on
                {
                    currentCompressionRatio = currentCompressionRatio + attackIncrement;
                    outputSample = (int) (input[n] / currentCompressionRatio);
                } else {
                ...
                if (Math.abs(getRegionMax(input, n)) > (int) thresholdLevel)  //switch on
                {
                    currentCompressionRatio = currentCompressionRatio + attackIncrement;
                    outputSample = (int) (input[n] / currentCompressionRatio);
                } else {
                ...
                if (Math.abs(getRegionAverage(input, n)) > (int) thresholdLevel)  //switch on
                {
                    currentCompressionRatio = currentCompressionRatio + attackIncrement;
                    outputSample = (int) (input[n] / currentCompressionRatio);
                    System.out.println("currentCompressionRatio = " + currentCompressionRatio);
                    System.out
                            .println("Math.abs(getRegionAverage(input,n) = " + (Math.abs(getRegionAverage(input, n))));
                    System.out.println("input[n] = " + input[n]);
                    System.out.println("outputSample = " + outputSample);
                } else {
                    currentCompressionRatio = currentCompressionRatio - releaseIncrement;
                    outputSample = input[n];
//					System.out.println("Region Average False");
                }
            }
