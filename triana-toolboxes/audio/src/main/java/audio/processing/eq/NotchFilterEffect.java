...
public class NotchFilterEffect extends AudioEffect16Bit {
    ...
    public NotchFilterEffect(int forwardMemSize, float frequency, boolean chunked, float delayInMs, float sampleRate,
    ...
    public short[] process(short[] input) {
        ...
        if (chunkedData == false) {
            outputData = notchFilterProcess(input);
            System.out.println("chunked off");
        } else {
            outputData = notchFilterProcessChunked(input);
    ...
    public short[] notchFilterProcess(short input[]) {
    ...
    public short[] notchFilterProcessChunked(short input[]) {
