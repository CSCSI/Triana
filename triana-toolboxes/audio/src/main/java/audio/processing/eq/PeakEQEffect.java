...
public class PeakEQEffect extends AudioEffect16Bit {
    ...
    public PeakEQEffect(int forwardMemSize, float frequency, boolean chunked, float delayInMs, float sampleRate,
    ...
    public short[] process(short[] input) {
        ...
        if (chunkedData == false) {
            outputData = peakEQProcess(input);
            System.out.println("chunked off");
        } else {
            outputData = peakEQProcessChunked(input);
    ...
    public short[] peakEQProcess(short input[]) {
        ...
        // double slope = 1;
//		double r = 0.5;
        double A = Math.sqrt(Math.pow(10, ((dbGainLevel) / 20)));
...
//        double Q = 1;
    ...
    public short[] peakEQProcessChunked(short input[]) {
