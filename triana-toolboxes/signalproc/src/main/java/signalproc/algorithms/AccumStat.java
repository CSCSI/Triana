 ...
 * are to output one statistic (the mean) averaged over the 10 most recent input sets.
 * <p/>
 * The unit outputs simultaneously (from an appropriate number of output nodes) a collection of data sets of the same
 * type as the input, one for each desired statistic (eg, mean, variance, etc.) The output sets contain the statistics
 * of each of the dependent data arrays of the input sets that are of arithmetic type. The statistics are computed
 * element-by-element, so that the output arrays have the same dimensionality and size as the input arrays. Dependent
 * data that are not arithmetic arrays (numbers) are passed directly through to the output.
 * <p/>
 * The unit can produce mean, variance, skewness (as the third moment, not normalized by the variance), kurtosis (again
 * not normalized), and higher moments up to order 8.  Data are output in the order of the moment: node 0 = mean, node 1
 * = variance, node 2 = skew, node 3 = kurtosis, etc.  Apart from the mean, the formula for the nth moment is < (data -
 * mean)^n >, where <..> indicates an average over the accumulated sets. Note that the moments are not normalized by the
 * variance, as is the case for the usual definition of skew and kurtosis.
 * <p/>
 * The accumulation continues in either single-step or continuous mode, as long as the network of units is not reset. At
 * the beginning of a sequence, before the number of input sets reaches the number given in the parameter window, the
 * output after each computation is a correct average of the sets so far received. After the number of inputs has
 * reached the desired number, then the average is made over the appropriate number of the most recent input sets. If
 * the number of moments is increased during the computation then the output for the extra moments will not be exactly
 * correct until the desired number of inputs has been reached.
 * <p/>
 * The algorithms saves a rolling queue of input data sets with a length equal to the number being averaged, so if input
 * sets are large or the number to be averaged is large then the memory storage requirements can grow rapidly. Users
 * should exercise caution.
 * <p/>
 * If the number of data sets to accumulate is changed in the parameter window during a sequence of computations, the
 * unit will behave correctly, either accumulating more (if it is increased) or forgetting the earliest ones (if it is
 * decreased). If the number of moments to be output is reset in the parameter window to a number that is more than the
 * current number of output nodes, then the number of nodes is automatically increased.  If number of nodes is larger
 * than the number of moments to be output, then the extra nodes are used to duplicate output data, just as for simple
 * units that produce only one output. Data are output to the excess nodes in a cyclic manner: eg if there are 2 moments
 * and 4 nodes the data will be output as node0:mean, node1:variance, node2:mean, node3:variance. If the network is
 * reset then the accumulated data will be lost and the averaging will start anew.
 *
 * @author B.F. Schutz
...
*/
                        ...
                        if (kc == 0) {
                            ...
                            ((GraphType) outputs.get(0)).setDataArrayImag(temp.restoreArray(), dv);
                        }
                        ...
                        if (highestPower > 1) {
                            ...
                            if (kc == 0) {
                                ((GraphType) outputs.get(1)).setDataArrayReal(temp.restoreArray(), dv);
                            } else {
                                ((GraphType) outputs.get(1)).setDataArrayImag(temp.restoreArray(), dv);
                            }
                        }
                        ...
                        if (highestPower > 2) {
                            ...
                            if (kc == 0) {
                                ((GraphType) outputs.get(2)).setDataArrayReal(temp.restoreArray(), dv);
                            } else {
                                ((GraphType) outputs.get(2)).setDataArrayImag(temp.restoreArray(), dv);
                            }
                        }
                        ...
                        if (highestPower > 3) {
                            ...
                            if (kc == 0) {
                                ((GraphType) outputs.get(3)).setDataArrayReal(temp.restoreArray(), dv);
                            } else {
                                ((GraphType) outputs.get(3)).setDataArrayImag(temp.restoreArray(), dv);
                            }
                        }
                        ...
                        if (highestPower > 4) {
                            ...
                            if (kc == 0) {
                                ((GraphType) outputs.get(4)).setDataArrayReal(temp.restoreArray(), dv);
                            } else {
                                ((GraphType) outputs.get(4)).setDataArrayImag(temp.restoreArray(), dv);
                            }
                        }
                        ...
                        if (highestPower > 5) {
                            ...
                            if (kc == 0) {
                                ((GraphType) outputs.get(5)).setDataArrayReal(temp.restoreArray(), dv);
                            } else {
                                ((GraphType) outputs.get(5)).setDataArrayImag(temp.restoreArray(), dv);
                            }
                        }
                        ...
                        if (highestPower > 6) {
                            ...
                            if (kc == 0) {
                                ((GraphType) outputs.get(6)).setDataArrayReal(temp.restoreArray(), dv);
                            } else {
                                ((GraphType) outputs.get(6)).setDataArrayImag(temp.restoreArray(), dv);
                            }
                        }
                        ...
                        if (highestPower > 7) {
                            ...
                            if (kc == 0) {
                                ((GraphType) outputs.get(7)).setDataArrayReal(temp.restoreArray(), dv);
                            } else {
                                ((GraphType) outputs.get(7)).setDataArrayImag(temp.restoreArray(), dv);
                            }
                        }
                    }
                }
